...
from app.models.jwt_model import RevokedToken
from datetime import datetime, timezone
from app.services.utills import convert_utc_to_ist
...
def is_token_revoked(db: Session, token_id: str):
    return (
        db.query(RevokedToken).filter(RevokedToken.token_id == token_id).first()
        is not None
    )
